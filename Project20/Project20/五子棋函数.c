...
//***********************************************************************************************************************************************
//根据规定规则来判断得分
//1.得分标准规则：没有棋子直接下棋，有1棋子得1分，（一）冲2得1分（至少一条线被堵着），（二）活2得2分（一个子且没有一条线没被堵着），（三）冲3得3分（两个子但一边被堵着）
//                                                （四）活3得4分(两个子)，（五）双活3得5分（四个字），（六）活4得5分（三个字）。
...
int IsRuleComScore(char arr[ROW][COL],int row,int col,int MaxComScore)
{
	...
	for (row = 1; row <ROW - 4; row++)
	{
		...
		{
			if (   (arr[row][col + 3] == arr[row + 1][col + 2])
				&& (arr[row][col + 3] == arr[row + 2][col + 1])
				&& (arr[row][col + 3] == arr[row + 3][col])
				&& (arr[row][col + 3] == '0')
				&& (arr[row - 1][col + 4] == ' ')
				&& (arr[row + 4][col - 1] == ' '))
			{
	...
	for (row = 0; row < ROW; row++)
	{
		for (col = 1; col < COL - 4; col++)
		{
			if ((arr[row][col] == arr[row][col + 1]) && (arr[row][col] == '0') &&
				(((arr[row][col - 1] == arr[row][col + 2]) && (arr[row][col - 1] == arr[row][col + 3]) && (arr[row][col - 1] == arr[row][col + 4])) ||
				((arr[row][col - 1] == arr[row][col + 2]) && (arr[row][col - 1] == arr[row][col - 2]) && (arr[row][col - 1] == arr[row][col - 3]))) &&
				(arr[row][col - 1] == ' '))
			{
				MaxComScore = 4;
				return MaxComScore;
	...
	for (col = 0; col < COL; col++)//for (col = 0; col < COL; col++)
	{
		for (row = 1; row < ROW - 4; row++)
		{
			if ((arr[row][col] == arr[row + 1][col]) && (arr[row][col] == '0') &&
				(((arr[row - 1][col] == arr[row + 2][col]) && (arr[row - 1][col] == arr[row + 3][col]) && (arr[row - 1][col] == arr[row + 4][col])) ||
				((arr[row - 1][col] == arr[row + 2][col]) && (arr[row - 1][col] == arr[row - 2][col]) && (arr[row - 1][col] == arr[row - 3][col]))) &&
				(arr[row - 1][col]==' '))
			{
				MaxComScore = 4;
				return MaxComScore;
...
void ComputerMove(char arr[ROW][COL],int row, int col)
{
	...
	int MaxPlaScoreRow = 0;//记录玩家的最高分时的坐标
	...
	while (1)
	{
		...
		else
		{
			...
			//}
			ComScore = IsRuleComScore(arr[ROW][COL], ROW, COL, ComScore);
			PlaScore = IsRuleComScore(arr[ROW][COL], ROW, COL, ComScore);
			//for (row = 0; row < ROW; row++)
			//{
			//	for (col = 0; col < COL; col++)
			//	{
			...
			else
			{
				...
				for (row = 0; row < ROW; row++)
				{
					for (col = 0; col < COL; col++)
					{
						...
						{
							arr[row][col] = '*';
							PlaScore = IsRulePlaScore(arr[ROW][COL], ROW, COL, PlaScore);
				...
				arr[MaxPlaScoreRow][MaxPlaScoreCol] = '*';
				break;
			}
		}
	}
